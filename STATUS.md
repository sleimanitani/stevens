...
**Next: step 3d** — `forge_power(manifest)`. Builds the runtime artifact for a power per its `modes:` + `runtime:` block (webhook → uvicorn unit; listener → service unit; polling → service+timer; request-based-only → no unit). Wires bus subscriptions per the manifest. Runs the manifest's bootstrap hook. Returns `ForgeResult`. Idempotent.
