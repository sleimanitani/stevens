...
| 2026-05-03 | _pending_ | `v0.11` step 3d — `forge_power(manifest)`. New `demiurge/pantheon/hephaestus/forge.py`: ForgeResult/ForgeAction/ForgeError; webhook → uvicorn unit; listener → `python -c importlib.import_module(...)` unit; request-based → no unit; polling → deferred note. Best-effort bootstrap hook (sync + async). Idempotent + differential-update detection. 837 passed, 2 skipped (+14 unit tests). |
...
**Step 3d shipped** (this commit, 2026-05-03): `forge_power(manifest)` — turn a power manifest into systemd unit(s) + run the bootstrap hook. Idempotent. Differential re-forge detected.

**Next: step 3e** — `forge_mortal / forge_beast / forge_automaton`. Mortal forge is the heaviest: provision agent identity, mint creature_id (UUIDv7), create `mortal_<id>` Postgres schema, compose `MortalContext` (blessed ToolRegistry + LLM stub + memory stub + bus + audit), wire observation feed writer into the supervisor, attach Enkidu audit-angel placeholder (in-process projection of the feed into today's audit log). Plus a real `scheduler` Automaton as proof-of-Automaton.
