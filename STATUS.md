...
**Step 3e.1 shipped** (this commit, 2026-05-03): manifest schema extension (Beast + Automaton kinds) + `forge_mortal(manifest, *, instance_id, …)`.

**Next: step 3e.2** — `forge_beast(manifest, ...)` + `forge_automaton(manifest, ...)`. Simpler shapes (no LLM, no memory; Automaton has no agency). Compose BeastContext / AutomatonContext.
