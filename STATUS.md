...
| 2026-05-03 | _pending_ | `v0.11` step 3e.2 — `forge_beast(manifest, ...)` + `forge_automaton(manifest, ...)`. Factored shared `_forge_creature` helper; all three Creature forges share one implementation. Per-kind pg-schema prefixes (`mortal_<id>` / `beast_<id>` / `automaton_<id>`). 867 passed, 2 skipped (+14 unit tests). |
...
**Step 3e.2 shipped** (this commit, 2026-05-03): `forge_beast` + `forge_automaton`. Factored a shared `_forge_creature` helper; per-kind pg-schema prefixes.

**Next: step 3e.3** — audit-angel placeholder. `AuditAngel` class implementing the `Angel` ABC; projects observation-feed `tool.call.*` events into the existing tamper-evident audit log. Same behavior as today's audit writer; refactored to live behind the Angel API so v0.13 can promote to out-of-process without changing what it produces.
