...
**Step 4 shipped** (this commit, 2026-05-03): Hades (underworld) — archive functions for Power/Mortal/Beast/Automaton, plus stubs for fade/exile/ragnarok Pantheon-member transitions.

**Steps 3 + 4 of v0.11 are complete.** Hephaestus and Hades are both real now (modulo the deferred bits each notes).

**Next: step 5** — `demiurge powers` CLI. Wire entry-point discovery (step 2) + Hephaestus.forge_power (step 3d) + Hades.archive_power (step 4) into the operator-facing `demiurge powers` subcommands: `list` / `registry` / `install <name>` / `uninstall <name>` / `show <name>`. Plus deprecated-alias `demiurge channels …` that calls into powers with a deprecation warning.
