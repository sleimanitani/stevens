...
**v0.10-bootstrap is complete** (2026-05-02). All 8 steps shipped. The new install path is `uv run demiurge bootstrap` → run the printed sudo block → re-run bootstrap → `demiurge secrets init` → `systemctl --user start demiurge-security`. Native Postgres 16 + systemd user units, no docker, no `usermod -aG docker`.

**Active milestone moves to `v0.11-plugins`** — see `plans/v0.11-plugins.md`. Channels + Mortals migrate from in-tree directories into per-plugin packages under `plugins/`, discoverable via Python entry points. Operator surface: `demiurge channels install <name>` and `demiurge hire spawn <spec>`. The runbooks under `docs/runbooks/` will get re-pointed at `demiurge channels install` once the plugin shape lands; until then they remain the canonical onboarding flow.
