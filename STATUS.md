...
| 2026-05-03 | `5be7565` | `v0.11` step 6 — `demiurge hire` CLI. New `cli_hire.py` with `list`/`registry`/`show`/`spawn`/`install` (alias)/`retire` subcommands wiring discovery + Hephaestus.forge_mortal/beast/automaton + Hades.archive_*. `pause`/`resume` are deferred-stubs until step 7 (supervisor). Placeholder dispatchers wired — error loudly if anyone tries to use a blessed tool before the supervisor is live. 959 passed, 2 skipped (+22 unit tests). |
| 2026-05-03 | `aa32e01` | `v0.11` step 5 — `demiurge powers` CLI. New `cli_powers.py` with `list`/`registry`/`show`/`install`/`uninstall` subcommands wiring discovery + Hephaestus.forge_power + Hades.archive_power. Deprecated alias `demiurge channels list` prints a deprecation note and delegates. 937 passed, 2 skipped (+17 unit tests). |
| 2026-05-03 | `26ecb99` | `v0.11` step 4 — Hades (underworld) archive functions. Symmetric to Hephaestus's forge: `archive_power(name)` removes systemd unit; `archive_mortal/beast/automaton(creature_id)` removes agent identity + policy block + key/env files, moves observation feed to archive root, renames per-creature pg schema (or drops with `drop_data=True`). Fail-tolerant (vs forge's fail-fast). Idempotent. Pantheon-member stubs (`fade`/`exile`/`ragnarok`) print manual-intervention notes. **Steps 3 + 4 complete.** 920 passed, 2 skipped (+14 unit tests). |
| 2026-05-03 | `5af578d` | `v0.11` step 3e.4 — real scheduler Automaton. New `demiurge/creatures/scheduler.py`: `Scheduler(Automaton)` with subscription registry + `parse_interval` (s/m/h/d) + `subscribe`/`unsubscribe`/`tick()` that fires `creature.tick.<id>` bus events when intervals elapse. First-tick-fires-immediately sentinel; failed publishes recorded but don't stop the scheduler. **Step 3 of v0.11 is now fully complete (3a-3e.4).** 906 passed, 2 skipped (+25 unit tests). |
...
| ✓ | 8 | `62242a9` | migrate channels → powers (gmail/calendar/whatsapp-cloud/signal moved to `plugins/demiurge-power-<name>/`; discovery + CLI confirm all 4) |
| ✓ | 9 | `3507347` | migrate Mortals to plugins (email_pm + installer extracted to `plugins/demiurge-mortal-<name>/`; both discovered via `demiurge hire registry`) |
| ⏳ | 10 | — | runbook overhaul |
| ⏳ | 11 | — | acceptance gate |
