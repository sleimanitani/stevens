...
**Step 6 shipped** (2026-05-02, this commit): `compose.yaml` relocated to `dev/`, README + master runbook flow updated to use `stevens bootstrap`. Signal runbook got a transitional banner — its `signal-cli-rest-api` daemon still uses docker (under `dev/`) until a follow-up apt+systemd recipe lands.

**Next: step 7** — the runbook content overhaul. Rewrite the per-channel runbooks (`gmail.md`, `calendar.md`, `whatsapp-cloud.md`, `signal.md`) so the install steps say `stevens bootstrap` once and the rest is per-channel-specific (OAuth dance, webhook URL, etc). Also clean up `DEVELOPMENT.md` (8 stale `docker compose` references; mostly v0.0-skeleton fossil text — likely worth a wholesale rewrite or replacement with a pointer to README + runbooks).
...
- **`compose.yaml`:** moved to `dev/compose.yaml` in v0.10 step 6. Production install uses `stevens bootstrap`.
