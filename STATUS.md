...
| 2026-05-03 | _pending_ | `v0.11` step 5 — `demiurge powers` CLI. New `cli_powers.py` with `list`/`registry`/`show`/`install`/`uninstall` subcommands wiring discovery + Hephaestus.forge_power + Hades.archive_power. Deprecated alias `demiurge channels list` prints a deprecation note and delegates. 937 passed, 2 skipped (+17 unit tests). |
...
**Step 5 shipped** (this commit, 2026-05-03): `demiurge powers` CLI wiring discovery + forge_power + archive_power; deprecated `demiurge channels list` alias preserved.

**Next: step 6** — `demiurge hire` CLI. New top-level subcommand. `list` / `registry` / `install` / `spawn` / `show` / `retire` / `pause` / `resume`. Per-Mortal-id state in a `mortals` table (created by a new migration). `retire` calls Hades.archive_mortal.
