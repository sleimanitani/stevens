...
**Step 4 shipped** (2026-05-02, this commit): `stevens bootstrap` CLI. The orchestrator that composes Steps 1+2+3. End-to-end verified: `stevens bootstrap --dry-run` prints the full plan; a real run idempotently re-provisions, applies all 9 migrations, writes `~/.config/stevens/env`, and generates the 6 systemd unit files. The docker-group check fires per Principle 14 with a clear removal hint.

**Next: step 5** — docker-group refusal in `stevens doctor` (currently only `bootstrap` checks it). Extract `_in_docker_group()` into a shared `bootstrap.preflight` helper, wire `doctor` to call it as a non-blocking warning (operator may have docker for unrelated reasons; bootstrap hard-fails, doctor flags). Tiny step — mostly code-move + one extension to `doctor.py`.
