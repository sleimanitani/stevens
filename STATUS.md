...
| 2026-05-03 | _pending_ | `v0.11` step 3a — `shared.creatures` package. ABCs for Mortal/Beast/Automaton/Angel + frozen-dataclass contexts + observation-feed writer (UUIDv7, fcntl-locked appends, JSONL envelope) + `think` and `mortal.return` universal tools + `ToolRegistry` consumer-side types with contextvar dispatch. No god integration yet — pure types + the feed substrate. 773 passed, 2 skipped (+42 unit tests). |
...
**Step 3a shipped** (this commit, 2026-05-03): `shared.creatures` package. The plugin-author-facing contract — ABCs, contexts, observation feed, universal tools. No god integration yet.

**Next: step 3b** — `Blessing` / `GodlyBlessing` protocol with concrete in-memory mock-god tests. Forward-declared types (`Blessing`, `Denial`, `ToolRequest`, `AngelSpec`, `GodlyBlessing` Protocol) already landed in 3a; step 3b adds the dispatch helpers + an in-memory mock god for testing the bless/deny pathways end-to-end before wiring real gods in 3c.
