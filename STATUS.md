...
| 2026-05-03 | _pending_ | `v0.11` step 3e.3 — audit-angel placeholder. New `demiurge/pantheon/hephaestus/audit_angel.py`: `feed_event_to_audit_entry()` pure projection + `AuditAngel(Angel)` with `observe()` that pairs `tool.call.start` ↔ `tool.call.end` by correlation_id and writes via existing `AuditWriter`. First real Angel implementation. v0.11 placeholder runs alongside existing dispatch-side audit writes; v0.13 flips the switch + promotes to out-of-process. 881 passed, 2 skipped (+14 unit tests). |
...
**Step 3e.3 shipped** (this commit, 2026-05-03): audit-angel placeholder. First real Angel implementation; pairs `tool.call.start`↔`tool.call.end` events by correlation_id and writes via existing `AuditWriter`.

**Next: step 3e.4** — real `scheduler` Automaton implementation. Ticks per a manifest-declared cron, fires `creature.tick.<id>` bus events for polling-mode powers + Mortals that subscribe. Proves the Automaton kind end-to-end.
