...
| 2026-05-02 | _pending_ | `v0.11` step 1 — plugin manifest schema + parser. New `shared/src/shared/plugins/` subpackage; `Manifest` Pydantic model with the four-mode taxonomy (webhook/listener/polling/request-based); `RuntimeBlock` sub-models per reactive mode; cross-field validators enforcing kind/mode/runtime alignment + request-based-only-no-runtime + Mortal-only `powers` field + capability/secret shape. `load_manifest_from_text` + `load_manifest_from_yaml`. 715 passed, 2 skipped (+22 unit tests). |
...
**Step 1 shipped** (this commit, 2026-05-02): `shared.plugins.manifest` with the full Pydantic model + 22 unit tests covering valid + invalid cases.

**Next: step 2** — entry-point discovery. `shared/src/shared/plugins/discovery.py` wraps `importlib.metadata.entry_points` to scan installed `demiurge.powers` and `demiurge.mortals` plugins. Tests: install a fake plugin in a `tmp_path` venv, discover it.
