...
**Step 3b shipped** (this commit, 2026-05-03): blessing dispatcher (`dispatch.py` + 22 tests). Parallel `asyncio.gather` fan-out; defensive denial paths (god raises, god returns wrong type, prefix unrouted, god not installed); MockGod for test infrastructure.

**Next: step 3c** — tool routing + dispatch wiring. The hardcoded prefix→god route map for v0.11 (`gmail/calendar/whatsapp/signal → enkidu`, `web → arachne`, `pdf → sphinx`, `browser → janus`, `memory → mnemosyne`-stub, `say → iris`-stub, `zeus → zeus`-stub). Plus the tool-wrapper builder that takes a granted Blessing and produces a callable that validates the blessing on every dispatch and routes through the owning god. Wires Enkidu as the first concrete `GodlyBlessing` impl backed by its existing capability/policy machinery.
