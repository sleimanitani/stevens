...
uv run stevens passphrase remember         # opt-in: silent unlocks via OS keyring

# 3. onboard channel(s) — see the per-channel runbooks below
# 4. provision an agent and run it
uv run stevens agent provision email_pm --preset email_pm
uv run stevens agent run email_pm
```

When something doesn't work, run `uv run stevens doctor` for a
diagnostic with one-line remediations.

To start over, run `uv run stevens reset` (default = dry-run; pass
...
Quick discovery via the CLI: `uv run stevens channels list`.
...
2. **`channel_accounts` row per account.** Postgres-tracked; `stevens dep list` and the like read from here.
