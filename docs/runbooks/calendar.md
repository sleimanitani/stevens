...
- Demiurge already installed: `uv run demiurge bootstrap` succeeded, sealed store initialized. See [`README.md`](README.md) §"Fresh-install master flow".
- `gcloud` auth + Janus extra (same as Gmail).
- A public webhook URL to receive Calendar's push channels (separate path from Gmail; e.g. `https://demiurge.example.ts.net/calendar/push`).
...
```bash
# Same as the Gmail flow but with a distinct project id. Demiurge treats
...
uv run demiurge wizard google --project-id demiurge-calendar \
    --push-endpoint https://demiurge.example.ts.net/calendar/push
uv run demiurge janus run google_oauth_client --project-id demiurge-calendar
...
- **"Push channel expired."** Calendar push channels expire periodically (~1 week). The `calendar_adapter/watch_renew.py` cron renews them; make sure that's running. `demiurge doctor` will warn if a channel is past its renewal window.
