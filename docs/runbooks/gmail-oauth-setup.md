...
- `stevens` CLI available (`uv run stevens --help`).
...
jq -r '.installed.client_id'     ~/Downloads/client_secret_XXX.json \
  | uv run stevens secrets add gmail.oauth_client.id --from-stdin --metadata kind=oauth_client

jq -r '.installed.client_secret' ~/Downloads/client_secret_XXX.json \
  | uv run stevens secrets add gmail.oauth_client.secret --from-stdin --metadata kind=oauth_client
...
```bash
# Get the old id.
uv run stevens secrets list | grep gmail.personal.refresh_token
# Rotate — the new value comes in through the add_account flow; or paste by hand.
new_token="$(cat fresh_refresh_token.txt)"
echo -n "$new_token" | uv run stevens secrets rotate <old-id> --from-stdin
```

Revoke (lost device / compromised token):

```bash
uv run stevens secrets revoke <id>
